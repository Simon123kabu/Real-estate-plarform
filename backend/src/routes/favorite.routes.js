...
router.get('/:propertyId/status', isAuthenticated, getFavoriteStatus);

// POST /api/favorites/:propertyId  — toggle save/unsave
router.post('/:propertyId', isAuthenticated, toggleFavorite);

// DELETE /api/favorites/:propertyId — explicitly remove a favorite
router.delete('/:propertyId', isAuthenticated, removeFavorite);
